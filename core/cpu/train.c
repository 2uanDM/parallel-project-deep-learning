...
void init_nn(NeuralNetwork *nn, int input_size, int hidden_size,
             int output_size) {
...
double compute_loss(double *y_true, double *y_pred, int size) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    // printf("NaN detected at index %d: true_value=%f, pred=%f, term1=%f,
    // term2=%f\n", i, true_value, pred, term1, term2); return -1; // Return an
    // error code or handle it appropriately
    //}
...
// Compute gradients
void backward(NeuralNetwork *nn, double *input, double *y_true, double *dw1,
...
// Update parameters
void update_params(NeuralNetwork *nn, double *dw1, double *db1, double *dw2,
...
// Test accuracy
double test_accuracy(NeuralNetwork *nn, double *x_test, double *y_test,
                     int data_size) {
  int correct = 0;
...
// Train the network
void train(NeuralNetwork *nn, double *x_train, double *y_train, int epochs,
